...
0. One 0.1 scope line is unresolved: the **animation speed control** has been in scope since 2026-07-28, is not implemented anywhere, and was never dropped. It needs an owner decision — build it or strike it from Scope — before 0.1 can honestly be called finished. Do not silently drop it.
1. Owner hands-on QA is the remaining 0.1 *implementation* work, in this order: listen to the two reaction cues (never heard by anyone — they are measured non-silent, which is not the same as correct), drop the mascot from several heights to confirm the drop-and-roam feel, then the display matrix. The dismiss/quit transition and its two cues were owner-approved on 2026-08-01, happy path only — do not read that as covering Reduce Motion, re-summon mid-poof, or the second-Quit escape hatch. Everything else is built and verified — the Claude Code adapter was observed driving the mascot from a real session on 2026-07-30 (`started -> active -> completed -> stopped`). Codex shares the adapter but has never been run live, and `waiting`/`failed` remain fixture-only, so do not describe either as proven.
