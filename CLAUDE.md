...
- The menu bar item must stay an **app-owned `NSStatusItem`** created in `AppDelegate.installStatusItem()`, with its own `autosaveName` and `isVisible` set unconditionally. Do not go back to a SwiftUI `MenuBarExtra`. A `MenuBarExtra` obeys the visibility macOS remembers for it, and on 2026-08-03 a stray Command-drag of the icon off the menu bar left Control Center re-sending `NSStatusItemChangeVisibilityAction` on every launch — with that scene the app's only one, SwiftUI terminated the process ~0.1s in, so Dock Pet could not be launched, controlled, or quit, and wrote no crash report. Pinning `isInserted` kept the process alive but did not restore the icon. This app's icon is its only escape hatch, so it is never allowed to stay hidden.
