...
- **The idle-CPU gate is still failing.** It measured 3.40% median with one mascot against a <1% target on 2026-08-02. The animation loop was throttled on 2026-08-05 (ambient tick 20 → 12 Hz, timer tolerance, and a pause while the panel is occluded), but that change is **unmeasured** — summoning a mascot needs a menu bar click, which the session's tooling could not perform against an `LSUIElement` app. Arithmetic predicts roughly 2.2%, still short. Nobody has looked at the 12 Hz walk either.
