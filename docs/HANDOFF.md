...
- Success and failure each play a short synthesized cue, generated by `tools/author_sound_effects.py` and bundled as WAVs. Only those two states make sound, only while the mascot is summoned, and a persisted menu toggle mutes them.
...
- The dismiss transition, the quit farewell, and the two transition cues are owner-approved as of 2026-08-01, on the happy path only. The app icon renders correctly when extracted from the built bundle but has not been confirmed in Finder.
