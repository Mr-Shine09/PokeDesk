...
- Four short synthesized cues exist as of 2026-08-01: `success`, `failure`, `summon`, and `dismiss`, all generated by `tools/author_sound_effects.py` and bundled as WAVs. One persisted menu toggle mutes all of them. The summon and dismiss cues were owner-approved on 2026-08-01. The two reaction cues have still never been heard by anyone.
...
- Four cues exist as of 2026-08-01: `success`, `failure`, `summon`, and `dismiss`, all synthesized by `tools/author_sound_effects.py`. One menu toggle silences all of them. The summon and dismiss cues were **heard and approved by the owner on 2026-08-01**. The two *reaction* cues have still never been heard by anyone — that gap is unchanged and remains the oldest unverified claim in the project.
...
- 184 Swift package tests pass with a **second display attached**, where `aDropPastAnEdgeIsClampedBackIntoView` fails; that failure reproduces on `origin/main` and is a real multi-display defect rather than a regression. The unsigned Debug Xcode build succeeds. Run `swift package clean` before believing a failure that appears right after a stored property is added to a public struct.
...
7. The event path is complete and live-verified for Claude Code. Drop-and-roam was owner-approved on 2026-08-01. The animation speed control was deferred out of 0.1 by owner decision the same day. The only remaining 0.1 work is owner hands-on QA: **listen to the two reaction cues** (nobody has ever heard them — they are measured non-silent, which is not the same as correct), then the display matrix. The full list is in `docs/QA_CHECKLIST.md`.
...
| Atlas contract and output | `art/animation/` | Revision 6; `hand-sign` and `poof` dismiss rows added 2026-08-01 |
