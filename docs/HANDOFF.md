...
- Five short synthesized cues exist: `success`, `failure`, `summon`, and `dismiss` as of 2026-08-01, and `waiting` added 2026-08-21, all generated by `tools/author_sound_effects.py` and bundled as WAVs. One persisted menu toggle mutes all of them. The first four were owner-approved on 2026-08-01; **`waiting` has not been heard by the owner yet.**
...
- Five cues exist: `success`, `failure`, `summon`, and `dismiss` as of 2026-08-01, plus `waiting` as of 2026-08-21, all synthesized by `tools/author_sound_effects.py`. One menu toggle silences all of them. **The first four were heard and approved by the owner on 2026-08-01, and `waiting` on 2026-08-21** — the sound itself, from Preview State; its firing against a real permission prompt is still unobserved. It is a descending two-note knock repeated four times over 4.66 seconds, tapering to a stop — deliberately not a rising run, because `success` and `summon` already are. The length was an owner request after hearing a 0.71s first cut; it is also why the cue is cut short when any other state reaches the screen or the mascot is dismissed.
