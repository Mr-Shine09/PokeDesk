...
- Dismissing plays a matching 1.1-second exit added 2026-08-01 at owner request: the mascot forms a two-handed ninja seal (atlas row 14, `hand-sign`) and vanishes in a code-rendered smoke poof, and the panel is ordered out only when that finishes. Reduce Motion substitutes a 0.3-second stationary fade. **Nobody has watched this on screen yet** — it is unit-tested and built, nothing more.
...
- Atlas revision 5 contains 15 rows at `768x1680`, adding the `hand-sign` dismiss seal on 2026-08-01. Revision 4 replaced the sit-shake ledges with small freestanding chairs. All rows validate structurally.
- 183 Swift package tests pass, and the unsigned Debug Xcode build succeeds. Run `swift package clean` before believing a failure that appears right after a stored property is added to a public struct.
...
| Atlas contract and output | `art/animation/` | Revision 5; `hand-sign` dismiss row added 2026-08-01 |
...
- The dismiss transition (ninja seal plus smoke poof) and the new app icon have never been looked at on screen. Both are covered by automated checks only, and automated checks cannot judge either one.
