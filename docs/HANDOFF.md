...
- Dismissing plays a matching 1.1-second exit added 2026-08-01 at owner request: the mascot forms a two-handed ninja seal (atlas row 14, `hand-sign`) and vanishes under a pixel smoke cloud (atlas row 15, `poof`), and the panel is ordered out only when that finishes. Quit plays the same farewell when a mascot is on screen. Reduce Motion substitutes a 0.3-second stationary fade. **Nobody has watched this on screen yet** — it is unit-tested and built, nothing more.
- Four cues exist as of 2026-08-01: `success`, `failure`, `summon`, and `dismiss`, all synthesized by `tools/author_sound_effects.py`. One menu toggle silences all of them. The two new ones have never been heard, the same gap the reaction cues already had.
...
- The dismiss transition (ninja seal plus pixel smoke poof), the quit farewell, the two new cues, and the app icon have never been seen or heard on screen. All are covered by automated checks only, and automated checks cannot judge any of them.
