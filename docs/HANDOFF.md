...
- The 197-test Swift package suite has 196 passes with a **second display attached**; only `aDropPastAnEdgeIsClampedBackIntoView` fails. That failure reproduces on `origin/main` and is a real multi-display defect rather than a regression. The Xcode build succeeds. Run `swift package clean` before believing a failure that appears right after a stored property is added to a public struct.
...
- `WindowCoordinator` resolves screen bounds through `panel.screen ?? NSScreen.main`, which follows keyboard focus. With two displays a drop past an edge is not reliably clamped back into view. Found 2026-08-01; pre-existing.
