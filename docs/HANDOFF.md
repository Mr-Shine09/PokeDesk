...
- 187 Swift package tests pass, and unsigned Debug and Release Xcode builds both succeed. Run `swift package clean` before believing a failure that appears right after a stored property is added to a public struct.
...
- Dock auto-hide, multiple displays, full-screen Spaces, non-Retina, sleep/wake, and screen-lock behavior need broader hands-on coverage. Left/right Dock placement is no longer in scope: `DockGeometry` anchors to the screen's bottom edge regardless of Dock position. A user-dropped height overrides that default and is re-clamped rather than discarded when the display configuration changes; nobody has yet dragged the mascot between two physical displays and watched what happens.
- *Which* display placement is measured against became deterministic on 2026-08-01. `MascotWindow` no longer reads `NSScreen.main` anywhere: it picks the display the panel covers most of, or the nearest one when it covers none. `NSScreen.main` names the display holding the key window, so with a second display attached the same drop landed in two different places depending on where the user had last clicked, and could clamp the mascot onto a display it was never dragged to. The geometry rules are unit-tested against synthetic arrangements; the hands-on multi-display matrix is still open.
