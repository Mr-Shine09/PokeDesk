...
- Two atlases share that contract: `mascot-atlas@2x.png` is the classic Claude/no-provider wardrobe; `mascot-atlas-codex@2x.png` keeps every pose and the lower outfit but adds dark sunglasses and an orange/white top. `MascotVisibleState.providers` selects the wardrobe; Codex wins the accent when both providers contribute, while diagnostics still name both.
...
7. The event path is complete and live-verified for Claude Code. The new Codex orange/sunglasses wardrobe is implemented and structurally verified but awaits owner visual approval and a live Codex run. All four sound cues were owner-approved on 2026-08-01. The only remaining 0.1 QA work is the Codex wardrobe visual check and the **display matrix** (auto-hide, multiple displays, Spaces, sleep/wake).
