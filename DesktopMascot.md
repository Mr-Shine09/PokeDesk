...
38. **Chat apps can drive the ideating pose since 2026-08-11, and the feature is not yet observed on screen.** The signal is the frontmost application's bundle identifier against a two-entry allowlist (`com.anthropic.claudefordesktop`, and ChatGPT's genuinely-named `com.openai.codex` — do not "correct" it). Three rules that are load-bearing and easy to undo by accident: it is **frontmost**, not running; it sits **below `working`** unlike manual ideating, because it is a guess rather than a statement; and **a provider with any live session ignores it entirely**, because the Claude desktop app hosts the chat and Claude Code behind one identifier and hooks must win. Browser tabs are permanently out of scope — detecting them means reading browsing history. **Do not tune the 120 s suppression window without the owner**, and do not test this by talking to Claude Code in the app under test: the first attempt did exactly that and the signal correctly stood down for the whole run.
