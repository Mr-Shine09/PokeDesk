...
18. The menu bar item is an **app-owned `NSStatusItem`**, not a SwiftUI `MenuBarExtra`, since 2026-08-03. A `MenuBarExtra` obeys the visibility macOS remembers, and a stray Command-drag off the menu bar made the app terminate itself ~0.1 s into every launch with no icon and no crash report. Do not convert it back. **The icon is nevertheless still missing**: a removal is keyed to the bundle identifier, in a system store the app cannot reach, proven by building one binary under two identifiers. The open decision is whether to change `PRODUCT_BUNDLE_IDENTIFIER`. Until then the app is reachable only by clicking a mascot that is already on screen. Verify the icon with `screencapture` of the menu bar strip — `CGWindowList` cannot see status items.
