...
### 2026-08-12 — Three attempts at one defect, and only the third had evidence

The day's last work, and the most instructive failure in this ledger so far.

- **The defect, reported by the owner in one sentence that turned out to contain the answer:** "Thinking pose works for new Claude chat only. Starting from the second question in the same chat, it did not think anymore."
- **Attempt one: re-register the observer when a response ends.** Reasoning: a probe run mid-stream proved the marker *was* present for later questions and absent in a settled baseline, so the tree was findable; toggling the feature off and on mid-stream restored the pose instantly, so re-registering must repair delivery. **Wrong.** `attach` ends in a direct `scheduleSearch`, so toggling forced *one look* while a stream happened to be in flight. **Forcing a look is not restoring callbacks.** The test that "confirmed" the hypothesis could not distinguish them, and nobody noticed until the fix shipped and failed.
- **Attempt two: register on the windows, not only the application element.** Reasoning: the app element hears app-level events — a window appearing, focus moving — which is exactly what opening a new chat produces, so the first question always worked. Plausible, well-argued, and **also wrong**, though it stays in the tree because it is correct as far as it goes. What made attempt two different is that it shipped **instrumentation alongside the fix**: a callback counter in the menu line.
- **The counter answered it in one test.** Roughly **eight callbacks per question, and none during streaming**. They arrive when the prompt is submitted, a search runs *before* the answer element exists, finds nothing, and nothing ever asks again. **The design's premise was false** — layout changes do not fire constantly while a response streams — and both earlier repairs were built on that premise rather than on a measurement.
- **The fix is a poll, which this project had ruled out in writing.** A 1 Hz search that runs only while the chat app is frontmost and nothing is generating. Owner decision, taken on the measurement. Two comments instructing the reader never to add a poll were **corrected in place rather than deleted** — including one written two commits earlier, by me, in this same session. Leaving the reasoning next to its refutation is the point.
- **Verified in one pass:** three questions in one chat all posed, all reported `generating`, all fist-pumped; the ambient walk stayed smooth while sitting in a long conversation; and the callback count was static while another app was frontmost, so the poll stops and the cost is bounded.
- **The reusable lesson:** a silent detector has two failure modes that print the same words — the search ran and missed, or nothing asked it to search. **Until the counter existed, no test could tell them apart**, so every hypothesis was unfalsifiable and two of them looked confirmed. Ship the instrument before the third guess, not after.
- **A second, unrelated hazard was found and cost the app its life.** Chasing the Accessibility grant that a reinstall voids, the assistant advised signing with the keychain's Apple Development identity. **Those certificates are revoked**, and macOS treats an app signed by a revoked certificate as **malware** — it showed "Malware Blocked and Moved to Trash" and deleted the installed app. `codesign --verify` passed throughout and the bundle satisfied its designated requirement; only `spctl --assess` reveals it, as `CSSMERR_TP_CERT_REVOKED`. **`security find-identity -v -p codesigning` reports these certificates as valid and does not check revocation** — that listing was the evidence trusted. Install ad-hoc with `CODESIGN_IDENTITY=-`; `docs/DEVELOPMENT.md` now says so, where it previously advised the opposite.
- **Exact next step:** merge the fix, then the core app smoke test, which is now the largest untouched block. Behind it: the two narrow chat rows (an untimed pose latency, and unticking mid-stream), the fixture-only `failed` state, and issue #11's Reduce Motion scope.
...
38. **Chat lifecycle detection reads the Claude window through the accessibility API, by owner decision on 2026-08-11, reversing this project's "no accessibility permissions" promise.** It is **off by default** and **was observed working end to end on 2026-08-11** — Thinker pose during a stream, success reaction on completion, back to strolling. **The negative and interaction rows were walked on 2026-08-12 and all pass** — a quiet chat produces no pose, a working Claude Code turn outranks a streaming response, an idle session does not block one, the navy mascot is unaffected, the pose holds for a whole stream, and the toggle survives relaunch. The feature is now proven in both directions rather than on a single positive run. **A defect found the same day and fixed on the third attempt:** only a conversation's *first* response was detected. See the day's final entry. **Two things remain open and are narrower than they look:** the Thinker pose's latency from Enter has never been *timed* (the pose has been seen repeatedly; the row claims under a second), and unticking the menu item **mid-stream** to watch the pose drop at once has not been reported — only the persisted choice has. What must not be undone by accident:
    ...
    - **There is a 1 Hz poll, it is deliberate, and removing it reintroduces the 2026-08-12 defect.** It runs *only* while the chat app is frontmost, the feature is on, and nothing is already generating; it hands over to the recheck timer once a stream is found, and stops when you switch away. **The source and `docs/QA_CHECKLIST.md` both used to instruct the reader never to add a poll**, and those comments are corrected in place rather than deleted so the reasoning sits beside its refutation. The premise they rested on — that layout changes fire constantly while a response streams — was measured false: roughly **eight callbacks per question, none during streaming**. They arrive when the prompt is submitted, a search runs before the answer element exists, finds nothing, and nothing asks again. Owner decision to accept the poll, 2026-08-12, taken on that measurement. Verified the same day that the walk stays smooth in a long conversation and the callback count is static while another app is frontmost.
    - **Keep the callback counter in the menu line.** It reads `Claude chat: <state> · <n> events` and nothing consumes it to choose a pose. It exists because a silent detector has two failure modes that print the same words — the search ran and missed, or nothing asked it to search — and **two repairs were written before it existed, both of which looked deduced and were guesses**. The first rested on toggling the feature mid-stream restoring the pose, which proves only that *forcing one look* works, not that callbacks were restored. If detection ever goes quiet again, read this number before forming a hypothesis: climbing means the search runs and misses, frozen means nothing is asking.
