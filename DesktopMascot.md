...
| Current gate | Owner installs the printed hook snippet and runs a real Claude Code or Codex session, which is the only thing that can prove the adapters work against live providers. Everything else in the path is implemented and verified. Hands-on QA also outstanding for the physical cursor-hanging feel and the display matrix |
...
| Provider adapters | Fixture tests and live smoke tests | Fixtures passed 2026-07-30: 23 adapter fixtures inside a 166-test suite, including a realistic `PreToolUse` payload proving no path, directory, command, or credential from the payload reaches the envelope. A synthesized full lifecycle produced the expected events against the running app. **Live smoke test against a real provider session has not happened**, so the documented hooks are still trusted rather than observed |
