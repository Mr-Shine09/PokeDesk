...
- **What that leaves as the next step:** Reduce Motion sign-off ([issue #11](https://github.com/Mr-Shine09/desktop-mascot/issues/11)) and the four dismiss edge cases — Reduce Motion, re-summon mid-poof, dismiss-while-paused, and the second-Quit escape hatch. All four are cheap and none needs an agent running. The README screenshot wants an owner capture and is now easy, since every state on the project page has been seen working.
