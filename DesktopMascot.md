...
- **The three on-screen observations remain open, and this session could not do them either.** `screencapture` from this process returns a blank frame — the agent's process has no Screen Recording permission — so visual verification is still an owner action. What changed is that it is now *possible*: the checks in handoff item 31 will exercise the real 2026-08-09 code for the first time.
...
- **Still unobserved: the waiting timeout.** It needs a Claude Code session in a normal permission mode, because `Notification` is what maps to `waiting` and a bypass-permissions session never fires one. That is a scheduling constraint, not an engineering one.
...
27. Keep the README's "Current limitations" section honest. It names the absence of a notarized build, the unobserved Codex on-screen reaction, and the fixture-only `failed` state, and those must not be softened to make the project page look better. Two of its claims changed on 2026-08-09 and the file was updated rather than left stale: the idle-CPU gate is **restated and passing**, not failing, and `waiting` is **no longer fixture-only**.
...
31. **Three behavior changes shipped on 2026-08-09 with no on-screen observation at all** — the waiting timeout, the ideating reorder, and the sleep schedule. Every one is proven by unit test and a green build only. Do not describe them as verified. The cheapest checks are: set the sleep hour to the current hour and watch the pet lie down; toggle Manual Ideating while an agent is running and look for the **Thinker pose and thought cloud**; leave a real permission prompt unanswered for more than two minutes and confirm the pet stays put. **They were not merely undone until 2026-08-10 — they were impossible**, because the installed bundle predated all three changes. It was reinstalled from `main` that day, so the checks now exercise the real code. **Two of the three passed on 2026-08-10** — sleep schedule (including persistence across relaunch) and the ideating reorder. The waiting timeout is the one still unobserved. **This item said "look for the lightbulb" until 2026-08-10, which is the `failure` marker, not ideating** — a cracked light bulb is row 6. An instruction naming the wrong animation makes a correct pass look like a failure and a real failure look like a pass; check a described animation against `art/animation/ATLAS.md` before putting it in a test script.
...
33. **This agent process cannot see the screen.** `screencapture` returns a blank frame from it — no Screen Recording permission — so visual verification is an owner action, not something a session can quietly do for itself. Do not record a capture taken this way as evidence; check that the image has content before believing it.
