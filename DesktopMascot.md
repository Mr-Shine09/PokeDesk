...
3. Treat atlas revision 5 as the current candidate: 15 rows, `768x1680`. Row 14 is the four-frame `hand-sign` dismiss seal added 2026-08-01, the six-frame `hanging` row remains at index 13 with a `(48, 4)` top grip anchor, and the directional sit-shake rows use a small freestanding chair.
...
5. Treat the current presentation as `96x112` points with a 10-point transparent Dock inset. Revision 5 appends one row inside the existing cell geometry; it changes nothing about the cell, the anchors, or any earlier row.
...
15. The dismiss transition and the app icon, both added 2026-08-01, have **never been looked at on screen.** Automated checks cover the frames, the timeline arithmetic, and the build; none of them can judge whether a smoke poof looks like a smoke poof. Ask the owner to watch a dismiss before treating either as done, and do not describe them as verified in the meantime.
16. Update this ledger before ending the next session.
17. Use `CLAUDE.md` and `docs/HANDOFF.md` as the maintainer onboarding entry points; keep them synchronized when architecture, commands, or asset contracts materially change.
