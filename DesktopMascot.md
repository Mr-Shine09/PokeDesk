...
- Verified, not assumed:
  ...
  - CI is written but **has never run** — the repository is still private and has no Actions history. `macos-15`, the runner's Xcode version, and `brew install xcodegen` are all unproven here. Expect the first run on the public repository to need a fix.
