...
| Current gate | **Release gates only — 0.1 feature work and hands-on QA are both closed.** The idle-CPU gate **failed** its first measurement on 2026-08-02 at a 3.40% median against a <1% target; the animation loop was throttled on 2026-08-05 (20 → 12 Hz ambient, timer tolerance, occlusion pause), which is **unmeasured and predicted to land near 2.2%, still short**. Also open: event-to-visible-state latency, Reduce Motion sign-off, signing, notarization, and packaging. Two hands-on claims remain: a real Codex session driving the navy mascot on screen, and a look at the 12 Hz walk |
...
- **The throttle is NOT verified against the gate, and on arithmetic alone it will not close it.** Being explicit because it would be easy to read this entry as a fix:
  - The 3.40% median was measured with a mascot **on screen**. Summoning one requires clicking the menu bar item, and Dock Pet is `LSUIElement`, which the computer-use tooling cannot resolve as an application — so this session could not summon a mascot and could not re-measure the figure that matters.
  - Arithmetic prediction, not a measurement: the 2026-08-02 run put the unsummoned floor at 0.40% and one mascot at 3.40%, so ~3.00 points are attributable to the loop. A 20 → 12 Hz cut is a 40% reduction in ticks, predicting roughly **2.2%** — still more than double the <1% gate. The occlusion pause contributes nothing in the measured condition, which is a visible mascot on the active Space. Treat 2.2% as a hypothesis to test, not a result.
  - What *was* measured today: the unsummoned app over 3 minutes, at ~0.55% for the first two minutes after launch and settling to ~0.07% thereafter. No regression against the 0.40% floor, but this exercises none of the animation loop.
...
- Next: owner looks at the 12 Hz walk and at dismiss/quit from another Space, then re-measures idle CPU with one mascot summoned. If it lands near the predicted 2.2%, the remaining choice is a further rate cut, coalescing both mascots onto one timer, or restating the gate after all — and that is a fresh owner decision, since "throttle the loop" has now been spent.
...
22. The animation loop was throttled on 2026-08-05: the ambient tick is **12 Hz**, transitions stay at 20 Hz, the timer carries tolerance, and the loop stops while the panel is occluded. Transitions and drags are exempt from the occlusion pause on purpose — `beginDismiss` orders the panel out from its completion and Quit waits on that, so pausing a dismiss would make the app unquittable. Do not extend the pause to cover them. The throttle is **unmeasured against the idle-CPU gate** and predicted to leave it failing at roughly 2.2%; the 12 Hz walk has also not been looked at by anyone.
