...
| Field | Current value |
| --- | --- |
| Project | PokeDesk — desktop mascot for macOS. Renamed from Dock Pet by owner decision on 2026-08-12, **in prose only**: the app bundle, the `dockpet-event` helper, and the bundle identifier keep their old names because installed hooks are keyed to them. See handoff item 40 |
...
| Current gate | **Release gates only — 0.1 feature work and hands-on QA are both closed.** The **idle-CPU gate was restated to <3% by owner decision on 2026-08-09 and now passes** at a 2.17% median with an Energy Impact of 3.70, in the band of `launchd` and `bluetoothd`; the old <1% figure was unreachable by tick rate without dropping below the sprite's own frame rate. **Event-to-visible-state latency was measured the same day and passes** at ~70 ms typical against a 500 ms gate. Remaining: Reduce Motion sign-off, signing, notarization, packaging — and signing is blocked on a Developer ID purchase, which is out of scope by owner decision. **No open engineering gate remains; everything left is observation or a purchase.** **The three 2026-08-09 behavior changes were all observed on screen on 2026-08-10 and all pass** — they had been unobservable until that day, because the installed bundle predated them. **A real Codex session drove the navy mascot on screen on 2026-08-10, closing the last 0.1 provider claim** — it went to its computer, played the success reaction, and the orange mascot was unaffected throughout. **The four dismiss edge cases were walked the same day and all pass.** Hands-on claims still open: the fixture-only `failed` state, issue #11's broader Reduce Motion coverage (summon and dismiss honor it; roaming does not), the core app smoke test, and the rest of the display matrix | **The display matrix closed 2026-08-11** — eleven of eleven rows on per-row evidence, after nine of them spent a day ticked by a 2026-08-02 blanket verdict nobody had walked. It found the only behavioral defect of the week: a dragged height discarded when its display was unplugged, now fixed, unit-tested, and re-observed on screen. **Two owner decisions the same day:** mascots float over full-screen apps on purpose, and the stationary pose stays the standing `idle` blink, leaving atlas rows 11 and 12 deliberately unused. The roaming toggle was renamed **Stay in One Place**. **What is left for 0.1 is the core app smoke test, issue #11's Reduce Motion scope, the fixture-only `failed` state, and signing — a purchase.** (The README screenshot closed the same day.) **Evening of 2026-08-11:** Claude chat detection and `EventSurface` were both watched working; three Claude chat rows and a terminal `codex` run remain unwalked, the latter untestable on this machine. **2026-08-11 also reversed a founding privacy promise by owner decision:** chat lifecycle detection reads one accessibility attribute on the Claude window and ships **off by default**. **It was observed working end to end later the same day** — Thinker pose while a response streamed, success reaction when it landed, then back to strolling — on the third attempt, after the first two were defeated by the test method rather than by the code. **Its negative and interaction cases were walked 2026-08-12 and all pass** — quiet chat, working agent outranking, idle session not blocking, navy mascot unaffected, pose held for a whole stream, choice surviving relaunch. The one failure that day was the reinstall voiding the Accessibility grant, which is expected and now confirmed rather than a defect. See the day's final entries and handoff item 38. **The same evening widened the event envelope for the first time**, adding a two-word `surface` so a ChatGPT chat turn thinks while a Codex agent run keeps typing; the ChatGPT desktop app is the Codex app, so the two were previously indistinguishable. Watched working; a terminal `codex` run is the one case this machine cannot test, having no CLI installed. See handoff item 39. **2026-08-12 closed the four issues that were already done** — #5, #7, #8, #9, each with what was dropped recorded on the issue. **#10 through #13 are the remaining release gates**, and #12's energy criterion is the only one with no engineering path left: the ambient loop is already at 12 Hz, the cost is near-linear in tick rate, and the owner restated the gate to <3% on 2026-08-09 rather than degrade the walk.
| Repository | [Mr-Shine09/PokeDesk](https://github.com/Mr-Shine09/PokeDesk) — **renamed from `desktop-mascot` on 2026-08-12**; old links redirect, and this ledger's historical entries keep them. **Public as of 2026-08-09**, flipped by the owner. Public-release documentation, licensing, and CI landed 2026-08-08; the username scrub across 12 tracked files and the first green CI runs followed on 2026-08-09. Still true and permanent: every commit carries the owner's real name and email, so the repository is public but not anonymous. The README screenshot landed 2026-08-11, and the README was rebuilt around v1.0 on 2026-08-12 with the long form kept as `DETAILED_README.md` |
...
- Short success and failure reaction cues (added 2026-07-30; this reverses an original non-goal, see below), and a `waiting` cue (added 2026-08-21).
...
- Cursor-following, free desktop roaming, speech, or notifications. **Sound effects were on this list and no longer are**: two short reaction cues for `success` and `failure` were added on 2026-07-30, muted by a persisted menu toggle and silent while dismissed, joined by summon and dismiss on 2026-08-01 and by `waiting` on 2026-08-21. Nothing else in the app makes sound, and the `waiting` cue is still not a notification — it comes from the mascot on screen, plays once on entering the state, and is silent while the pet is dismissed. "Free desktop roaming" still means what it always did — the mascot walks a bounded horizontal lane; letting the user choose that lane's height by dropping it is not free roaming.
...
The list above is the current contract. It differs from the 2026-07-28 original in six places, recorded here because a definition of done that drifts silently is worse than one that is merely out of date. Five are owner decisions; the sixth was an unanswered question, resolved on 2026-08-01.
...
| `waiting` cue added | **Widened** | 2026-08-21 | Owner asked for a sound on the waiting motion, "so that the user notices when the chat window is waiting for user's input"; widens the reaction-cue reversal from turn endings to the one moment the user has to act on |
...
**Chat-ideating is a second, weaker ideating rung, added 2026-08-11.** It sits below `working` — where manual ideating deliberately does not — because a chat is a conversation rather than reported work. Two different signals feed it, and the paragraph that described a third (the frontmost application's bundle identifier, with a blanket stand-down for any live session) is retired: that observer was replaced the same day and the suppression rule with it.

1. **The Claude chat window is generating**, read as one accessibility attribute on one element. Ordering alone protects the important case — a working agent outranks it, an idle session does not block it.
2. **A Codex turn driven from a desktop chat interface**, which is `EventSurface.desktopChat`. The ChatGPT desktop app is the Codex app, so a chat turn and a terminal agent run arrive as byte-identical hook events; the helper separates them from its own process ancestry. Owner rule: the chat interface thinks, Codex and the terminal type. A turn with no surface counts as a command line, so anything predating the field behaves exactly as before.

Browser tabs are out of scope permanently; detecting them means reading the user's browsing history.
...
| `waiting` cue | Deterministic regeneration, non-silent measurement, fires once per entry, bundled, mute toggle, and owner listening | **Built and heard 2026-08-21; length revised the same day.** Re-running `tools/author_sound_effects.py` leaves the four existing WAVs byte-identical and adds `waiting.wav`. The owner heard the first cut from Preview State and asked for four to five seconds, because 0.71s was easy to miss and being missed is the one failure this cue has. It is now **4.66s** — the same double knock four times, tapering 1.00 → 0.58, rests measured at 0.00/1.30/2.62/3.94s — at peak 0.220 FS, the same as the other four. It rides the same `onStateAppeared` path as the reactions, which fires only when the displayed state actually changes, so entering `waiting` knocks once and an unanswered prompt stays silent. `xcodebuild` puts `waiting.wav` in `Contents/Resources/`. **What the length introduced and what is now handled: the cue can outlive the state.** Any other state reaching the screen stops it, and so does dismissing the mascot. **Owner heard and approved it the same day** from Preview State on the installed build. That approval is of **the sound**, not of its firing conditions: it has still **not been heard against a real permission prompt**, and the cut-short behavior the length required has not been watched either. Both are open in `docs/QA_CHECKLIST.md` → "Waiting cue acceptance" |
...
- **Give `waiting` a cue.** Owner request: a sound on the waiting motion, "so that the user notices when the chat window is waiting for user's input", and explicitly different from the success cue. This is the first cue for a *state that persists* rather than for a moment, and it is defensible only because the trigger is the edge into the state: `AmbientAnimationController.adopt` calls `onStateAppeared` on a genuine change and nothing else, so a prompt left sitting produces one knock, not a repeating alarm. **The comment in `MascotSoundPlayer` arguing that waiting is continuous and therefore should stay silent is now wrong and was rewritten rather than left to contradict the code.**
- **Make it a falling knock rather than another rising run.** `success` and `summon` are both rising runs in the same register, and the owner's requirement was that this cue not be mistaken for success. Pitch alone is a weak separator when the sound is heard from another window, so the separator is rhythm: a descending fourth (B5 → F#5), a beat of silence, then the same pair again at 80%. It sits two octaves above `failure`, the other descending cue, and stays clean where that one beats and buzzes.
- **Run it for four and a half seconds, not for one.** Owner request the same day, after hearing the 0.71s first cut: make it last four to five seconds. The knock is not redesigned, it is **repeated** — four rings with a 0.62s rest between them, 4.66s in total. Each ring is quieter than the last (1.00, 0.86, 0.72, 0.58) so it reads as someone knocking and giving up rather than as an alarm escalating; **a flat repeat at full volume is a smoke detector, and the taper is what keeps it a pet.** The cue exists to be noticed, and 0.71s of it was easy to miss, which is the only way this particular cue can fail.
- **A cue that lasts four and a half seconds can outlive the state it announces, so cut it short.** A permission prompt answered in one second would otherwise leave the pet knocking at a door already opened. Any other state reaching the screen stops the knock, and so does dismissing the mascot. **This is a consequence of the length, not a separate feature** — at 0.71s the question never arose, and shortening the cue again would make the stop logic dead weight rather than wrong.
- **Do not gate it differently from the other reaction cues.** It stays behind the single Sounds toggle, behind `isVisible`, and inside the per-state coalescing window, so a dismissed mascot is silent and two mascots entering `waiting` together knock once. A separate toggle was considered and rejected: five cues with two switches is a preferences screen, and this app has a menu.
...
### 2026-08-11 — The third attempt: chat detection observed working, end to end

The close above stands as written (item 19); this is what happened after it. **The exact next step it named was taken and it passed.**

- **What the owner watched.** With the menu item ticked and Accessibility granted, a streaming response in the Claude desktop app put the orange mascot into the seated Thinker pose — captured mid-stream in a screenshot. When the response landed, the mascot played the success reaction once, stars and a fist pump, and returned to strolling. **Generating, completed, and the return to ambient were all seen in one run.** The feature this project reversed a founding privacy promise for now demonstrably works.
- **No rebuild was needed, and checking that first is what made the attempt cheap.** The installed bundle's mtime (16:00) predated the feature's commit (16:15), which is exactly the shape of item 32's staleness trap — but `strings` found `Currently streaming message` in the executable, because the install was taken from the worktree minutes before the commit. **The mtime comparison is a heuristic and the string search is the fact.** Had the mtime been trusted alone, the session would have opened with a reinstall that quit the app under test and cost a re-grant of Accessibility, for nothing.
- **`defaults read` proved the feature had never been switched on.** No chat-detection key existed in `com.mrshine09.dockpet`, so every launch since the change had constructed the watcher with `isEnabled: false`. That is consistent with "never observed working" being a fact about the switch rather than about the code, and it made the first setup step non-optional instead of a guess.
- **The run was clean of the two contamination shapes recorded earlier today.** No Claude Code session was live. The transcript's file-creation step was the Claude app's own tool, which fires no Dock Pet hook, so nothing refreshed a `claude-code` session and nothing outranked the chat signal. The removal of the suppression rule is what made the attempt reachable at all.
- **The diagnostic line was never read, and that was the right call.** It exists to separate "the marker was renamed" from "the signal arrived and was lost downstream" — a distinction that only matters when the pose fails. The pose is downstream of the line and cannot appear without the marker matching, so on a passing run the line is redundant. `docs/QA_CHECKLIST.md` now says to read it first on a **failing** run.
- **The success reaction was first reported as "the success, lightbulb motion", and the contradiction was resolved by asking rather than by reasoning.** The light bulb is `failure` (row 6); success is stars and a fist pump (row 5). The reducer cannot reach `.failure` from a chat — `completed` maps to `.success` and nothing else does — so the source would have supplied a confident correct answer for the wrong reason, by predicting the owner's screen from code. Asking which of the two animations was actually on screen cost one question and kept the evidence first-hand. **This is ledger item 31's mix-up arriving from the opposite direction: there, an instruction named the wrong animation; here, a report did.**
- **What is proven and what is not.** Proven: detection, the pose during a stream, the success reaction on completion, the return to strolling. Not exercised by this run and not to be inferred from it: that a quiet chat produces no pose, that a working agent outranks it, that an idle session does not block it, that the navy mascot is unaffected, and that the toggle drops the pose immediately. Those rows stay open in `docs/QA_CHECKLIST.md`. Pose latency from Enter was not timed either.
- **`README.md` said "never been seen working end to end" in a public file, and that stopped being true the moment the screenshot arrived.** Corrected the same hour. Item 27's rule cuts both ways and the good-news direction is the easy one to leave stale.
- **The menu label was stale and the owner approved the rename the same session.** It read **Think When Chat App Is Open**, with a VoiceOver label naming an app being "in front" — accurate for `ChatAppObserver` and wrong for `ChatActivityWatcher`, which replaced it at 16:15 and reacts to a response being produced. It also promised ChatGPT, which is unwired. It is now **Think While Claude Is Answering**, with the VoiceOver label, `Preferences.swift`'s doc comment, `AppDelegate`'s property comment, and the `README.md` menu table all corrected to match. **The Swift symbol `chatAppsDriveIdeating` and the defaults key `com.mrshine09.desktopmascot.chatAppsDriveIdeating` were deliberately left alone** — renaming the key silently resets every existing choice, the trap `reactionSoundsMuted` and `…roaming` also carry (item 37), and a symbol that matches its key is easier to trace than one that matches a label. The mismatch between the name and the behavior is now documented at the property instead of being fixed by a rename that would cost more than it buys.
- **The label was fixed one session after the signal changed, which is the fastest this project has closed that particular gap.** Item 37's cost was a feature rebuilt because a menu title misdescribed what already existed; here the same class of staleness lasted a few hours.
- **Exact next step:** the negative and interaction rows above, which are quick and all in one sitting — read a finished conversation and confirm no pose, run an agent turn and confirm `working` wins, then untick the menu item and confirm the pose drops. Only then ChatGPT or the `waiting` marker, which were correctly held behind this observation. Note that the rename ships in a build that is **not yet installed**: `~/Applications/Dock Pet.app` still carries the old label, and those rows do not need the new build, but a reinstall to pick it up will void the Accessibility grant and require re-granting (item 38).

### 2026-08-11 — ChatGPT needed no code: the app that would need it is Codex

- **The owner asked for chat detection to be extended to ChatGPT, and the right answer turned out to be that it already works.** The ChatGPT desktop app **is** the Codex app. That is why it ships as `com.openai.codex` — an identifier two sessions have recorded as looking like a mistake while explaining that it is not one, without either session working out *why*. Its turns fire the seven hooks in `~/.codex/hooks.json`, so ChatGPT use has been driving the navy mascot since the hooks were installed on 2026-08-02.
- **Watched twice, on the two cases that could have differed.** An agentic turn (tool calls, an approval prompt) put the navy mascot at its computer and through the success reaction; a purely conversational turn with no tools did exactly the same. The second was the one that mattered — the first could have been explained by the agent machinery alone.
- **The pose could not have come from anywhere else, and that is checkable rather than assumed.** ChatGPT's `streamingMarker` is `nil`, so the accessibility path emits nothing at all for `.codex`; and the only output the chat path has is `ideating`. A `working` pose requires a real `active` event. The cause was confirmed by reading the installed hook file, not inferred from the animation.
- **Building it would have been worse than useless.** Hooks report the true lifecycle — working, success, and `failed` if a turn ever fails — where an accessibility marker reports "probably thinking". And `working` outranks chat-ideating, so the AX pose would have been correct, invisible, and permanently untestable.
- **This is item 37 twice in two days.** Yesterday a menu label hid an existing feature and it was rebuilt as new work. Today an app identifier that looked like a typo hid a whole provider's coverage. Both times the check that would have caught it early was reading the code that would implement the feature — here, noticing that the bundle identifier in the chat allowlist was the same one the Codex hooks already served.
- **The probe was leaking and would have leaked into a second app.** `ChatAccessibilityProbe` printed `AXTitle` and `AXValue` in full for every non-text role — the exact rule that put a summary of the owner's conversation in a Desktop file yesterday, via a Claude button title. Fixed before it was ever pointed at ChatGPT: titles and values are now always a character count, and `AXDescription`/`AXHelp` print only up to 80 characters, since the Claude marker is 27 and a leaked sentence is longer. **The fix outlived the task that prompted it**, which is the argument for making it before the probe run rather than after.
- **`ChatApp.Descriptor` replaced the flat identifier map.** It carries the bundle identifier, display name, provider, and an **optional** streaming marker; `nil` means probe-only, and `ChatApp.watchable` is what the watcher iterates. ChatGPT is listed with `nil` and a comment saying it must stay that way. The type exists so that "known but not watched" is a state the code can express — the alternative was a plausible-looking constant that silently matches nothing, which is indistinguishable from a broken feature.
- **`ChatActivityWatcher` is now keyed by provider throughout** — observers, PIDs, throttles, recheck timers, completion holds, and the generating set. It currently drives exactly one app. **This is speculative generality and is recorded as such**: it was written before the ChatGPT finding, it builds and passes, and it was left in place rather than reverted because re-editing proven code for no user-visible gain carries more risk than the unused branch does. A future session may delete it back to Claude-only with no loss; that is a preference, not a fix.
- **One near-miss worth keeping.** While generalizing the tree walk, the `AXDocumentArticle` subrole check was dropped, which would have loosened Claude's proven match. It was caught reading the diff back, and the subrole is now a per-app field with Claude's value restored. **A refactor that generalizes a verified match is exactly where a verified match gets quietly widened.**
- Suite **223 → 225**: the watchable/probe-only distinction, and that no two chat apps share an identifier or a provider. The watcher and probe remain unreachable from the package test target — still a gap, unchanged.
- **Exact next step:** the negative and interaction rows for Claude chat detection, unchanged from the previous entry and now the only chat work left — a quiet chat produces no pose, a working agent outranks it, the toggle drops the pose. The owner is holding the PR until then. Note the bundle was reinstalled at 18:49 for the probe changes, so **Accessibility has to be re-granted** before any of those rows will pass.

### 2026-08-11 — The same finding, read correctly the second time: `EventSurface`

**The entry above concluded "there is nothing to build" and that conclusion was wrong.** Everything it established about the mechanism was right — the ChatGPT app is the Codex app, its turns fire the hooks, the pose was hook-driven. What it got wrong was the product judgement laid on top: it treated the mascot sitting at its computer during a ChatGPT chat as the correct outcome. **The owner's rule is that ChatGPT's chat interface thinks like Claude chat, and Codex and the terminal type.** The typing pose during a chat was the defect being reported, and it was recorded as a pass. The entry is kept because the mechanism it documented is what made the fix possible in one step.

- **The failure was reading the owner's report as confirmation.** Their words were "For ChatGPT chat, it is in typing motion, not ideating" — a description of what was wrong, phrased flatly, and it was filed as evidence that the provider worked. **A neutral report of what is on screen is data, not a verdict**, and the ledger's own habit of asking which of two animations was showing (used correctly two hours earlier on the lightbulb) is what should have run here.
- **The two are genuinely indistinguishable at the event level**, which is why this needed new machinery rather than a reducer tweak. Both fire `~/.codex/hooks.json`, both send `provider: codex`, and `HookPayload` extracts exactly two fields. The app receives a hashed session ID and a hook name; nothing in either says where the turn came from.
- **The discriminator is the helper's own process ancestry.** Confirmed from the live process table: the ChatGPT app runs `/Applications/ChatGPT.app/Contents/Resources/codex` as its own child, so a hook fired from the app has `ChatGPT.app` above it, while a terminal run has a shell and a terminal. `EventSurfaceDetector` walks parents from `getpid()`, bounded at 16 with cycle detection, and returns one of two words.
- **The alternative was offered and rejected with the tradeoff stated:** read the ChatGPT window the way Claude's is read. It needed a second Accessibility grant, a probe run, and a UI string any update can rename, and it would have required chat-generating to outrank `working` for one provider — which could mask a real terminal run happening at the same time. The owner chose ancestry.
- **This widens `EventEnvelope`, which item 8 forbids without a recorded product decision.** The decision is this entry. `surface` carries a two-case enum and nothing else: no path, command line, process name, or PID leaves the helper, and only the helper's own ancestors are examined. `envelopeExposesOnlyTheAllowlistedFields` failed the moment the field appeared — **the tripwire worked**, and updating it is the deliberate act a silent widening would have skipped.
- **Naming caught a real bug before it shipped.** The cases were first `app` and `terminal`. But the *Claude* desktop app hosts Claude Code, so a marker list keyed on "is this an app" would have turned every Claude Code turn into a Thinker pose. Renaming to `desktopChat`/`commandLine` made the question "is this an interface a person converses with", under which `Claude.app` is correctly absent from the markers — and a test now pins that absence with the reason.
- **The default is the old behavior, everywhere.** An unknown surface, an unrecognized string, an older helper, and any provider that never sets the field all reduce to `commandLine` and keep typing. The only way to reach the new pose is an affirmative match.
...
- Suite **241 → 244**.
- **Re-walked against the 19:40 build and it works.** The owner's verdict was "it worked" — **one overall verdict, not per-row commentary**, and `docs/QA_CHECKLIST.md` records it that way rather than as four narrated observations. Two rows do carry their own evidence, because each was reported on its own when it failed or first passed: the plain chat turn thinking, and the agent run typing after the tool-traffic rule was added.
- **One row cannot be walked here and stays unticked: a `codex` run in a terminal.** There is no CLI on this machine to run. Unit tests cover it and the default direction is the safe one, but it has not been watched, and an untickable row is more useful left open than quietly satisfied by the tests that already exist.
- **Exact next step:** the three Claude chat rows — a quiet chat produces no pose, a working agent outranks it, the toggle drops the pose — then the PR the owner is holding.

### 2026-08-11 — Session close (evening)

Four commits after the day's first close, all on `agent/chat-detection-observed`. The day's shape did not change: **what was shipped was smaller than what was corrected.**

- **Shipped and watched working:** chat detection for Claude, on its third attempt; and `EventSurface`, which makes a ChatGPT chat turn think while a Codex agent run keeps typing. Both were watched the same evening they were written, which is the first time in this ledger that has happened twice in one session.
- **Two features were asked for and neither existed as imagined.** "Implement chat detection for ChatGPT" turned out to be half already-solved — the app's turns had been driving the navy mascot since 2026-08-02 — and half a different problem entirely, which was that they drove the *wrong pose*. **Check what already happens before building what should happen**; that is item 37 for the third time in two days, and the third time it was an identifier or a label that hid a working feature.
- **The evening's real failure was mine and it was a reading failure, not a coding one.** The owner wrote "For ChatGPT chat, it is in typing motion, not ideating" — a flat description of a defect — and it was filed as evidence the provider worked, closing the task. **A neutral report of what is on screen is data, not a verdict.** Two hours earlier the same session had asked "stars, or a cracked bulb?" rather than assume, which is the habit that should have run here and did not.
- **Two corrections came from the owner watching, and both were of the same kind:** a rule that was right about the mechanism and too coarse about the product. Surface separated the app from a terminal but not the two modes inside the app. Each time the fix was one more condition, not a redesign, because the mechanism underneath was sound.
- **The privacy boundary was widened deliberately and the tripwire worked.** `envelopeExposesOnlyTheAllowlistedFields` failed the moment `surface` appeared; updating it is the visible act that a silent widening would have skipped. The field carries two words, decided in the helper from its own ancestry, and `README.md` discloses it as an always-on behavior rather than burying it under the opt-in chat feature.
- **Naming caught a bug that testing would have caught later and users would have caught first.** `app`/`terminal` would have turned every Claude Code turn into a Thinker pose, because Claude.app is an app. Renaming to `desktopChat`/`commandLine` made the marker list ask the right question, and a test now pins Claude.app's absence with the reason attached.
- **The probe's title leak was fixed before it was pointed at a second app**, and then the second app never needed probing. **The fix outlived the task**, which is the argument for making a safety fix at the moment you notice it rather than when it becomes load-bearing.
- Suite **223 → 244** across the evening. The watcher, the probe, and the AX walk remain unreachable from the package test target — unchanged, and still a gap.
- **Nothing is pushed since `5b2cb91`.** Three commits are local, and the PR is held by owner choice until the three Claude chat rows are walked.
- **Exact next step:** the three Claude chat rows, then push and open the PR. Behind that: the core app smoke test, which is still the largest untouched block in `docs/QA_CHECKLIST.md`, and the fixture-only `failed` state.

### 2026-08-12 — The rename, the cleanup, and a fix that had already shipped

A documentation and repository session. **No product behavior changed**, and the one code change is four lines.

- **The pet is now PokeDesk**, by owner decision, and the GitHub repository was renamed to match. The owner was told the name sits one syllable from Pokédex, a live Nintendo trademark, and that the realistic risk is a takedown after the project has an audience rather than a lawsuit; they kept it. That is recorded here so nobody re-raises it as a discovery. **The rename is prose only.** `Dock Pet.app`, `dockpet-event`, and `com.mrshine09.dockpet` keep the old strings because installed provider hooks point at those exact paths, and `EventSocketLocation.directoryName` still spells the older identifier still. The README says so under Install.
- **The README was rebuilt around v1.0** and the long-form original kept verbatim as `DETAILED_README.md`. Privacy, power, and requirements moved above the fold; the animation description moved down to sit beside the pictures; both tables gained mascot images from `docs/images/`. Writing the states table's new "When" column against `HookPayload.reaction(for:provider:)` corrected one row: **Success fires on `Stop`, which asserts the turn ended, not that everything inside it worked** — the README had implied the stronger claim.
- **"Browser chats coming soon" was cut rather than written.** Detecting a chat in a browser tab means reading the active tab's URL, which is browsing history. Promising it on the same page that promises the opposite would have been a lie with a deadline attached.
- **The repository went from 941 MB to 113 MB** — 622 MB of `.build`, and a 205 MB worktree removed after its contents were rescued.
- **A worktree held 333 lines of finished, uncommitted work from 2026-08-01**: `ScreenPlacement`, its 9 fixtures, and a full ledger entry, sitting outside git for eleven days. It is now commit `3b8877f` on `claude/eloquent-dhawan-d48dec`, pushed, and its 24 `MascotWindowTests` pass. **The rule that saved it is `CLAUDE.md`'s third: never discard an unfamiliar modification.** The rule that would have prevented the exposure is committing before leaving a worktree, which nothing enforces.
- **The day's real failure was mine, and it was the same reading failure this ledger has recorded before.** I checked whether `main` still had the defect by grepping for `NSScreen.main`, found it on line 85, and twice recommended opening a PR for the rescued fix. It was already fixed — **PR #35, eight days earlier** — and line 85 is only the no-displays-attached fallback. **I read a line and not the function around it.** Had the owner acted on that advice they would have gotten a five-file conflict and a request to re-decide something already settled. `git log -- <path>` was one command away and would have shown #35 by title.
- **The two placement designs genuinely disagree, and `main`'s is the newer decision.** The rescued branch chooses a display by pure geometry and deliberately carries no state; #35 remembers the last display the panel was genuinely on, and `7a898f3` then built on that memory. Neither is wrong; the branch is a superseded alternative, kept as a record and **not** to be merged.
- **One real gap survived both designs**, and it is [PR #50](https://github.com/Mr-Shine09/PokeDesk/pull/50): the walk rounded pixels against `panel.screen?.backingScaleFactor ?? 2` while the clamp resolved through `referenceScreen`. Those name different displays exactly when `panel.screen` is nil, which is the case `referenceScreen` exists for. `placementBackingScaleFactor` makes both read one source. 224 package tests pass; the Debug app build succeeds. **The new fixture holds trivially on one display** and only bites with two of differing scale attached, which this machine did not have — that is stated in the test and in the PR.
- **Only three of the seven "safe to delete" branches still existed.** Four had been deleted on the remote already; the local refs were stale and `list_merged_branches.sh` reported them faithfully from stale data. **Run it with `--fetch`, or the inventory is a snapshot of whenever you last fetched.** The chore itself had been sitting in this ledger since the 2026-08-01 entry.
- **`main` was pushed with two commits that no PR describes**, at owner instruction and after being told. Every other change in this repository arrived by squash-merged PR.
- **Issues #5, #7, #8, #9 were closed**, each with a comment separating what shipped from what was consciously dropped — the bottom-anchored Dock decision for #5, the never-edit-agent-config decision for #8 and #9, and the still-unobserved `failed` state for #9. Only #7 met every criterion as written. **#10 through #13 remain open and are the release gates**; #12's energy criterion is the one with no engineering path left, since the loop is already at 12 Hz.
- **Exact next step:** unchanged by this session — the three Claude chat rows and the core app smoke test. Then merge or close [PR #49](https://github.com/Mr-Shine09/PokeDesk/pull/49) and #50, and repoint the local remote (below).

### 2026-08-12 — Chat detection proven in both directions, after a permission cost one run

Later the same day, and it closes the caveat the previous entry left open.

- **PRs #49, #50, and #51 all merged; CI green on the merge commit; 245 package tests pass.** `main` now carries the rename, the rebuilt README, the backing-scale fix, and the rescued live-verification evidence. Two branches remain: `main`, and `claude/eloquent-dhawan-d48dec` as a record.
- **The app was reinstalled and both mascots were confirmed on screen** — orange at its computer because a real Claude Code session was driving it, navy standing `offline` because no Codex session existed and Stay in One Place was ticked. They rendered correctly on the **1x Dell**, which is the mixed-scale arrangement PR #50's own fixture could not exercise.
- **That check found a README defect nobody was looking for.** The Paused row illustrated itself with the `offline` sprite — the one carrying a `z`. The two are confusable precisely because a mascot with Stay in One Place ticked holds `offline` motionless, which *looks* paused. It was caught by rendering all 16 atlas rows against the contract rather than trusting the filename, and fixed with a fresh capture verified the same way. **A screenshot named after a state is not evidence of that state.**
- **B1 failed, and the cause was the Accessibility grant, not the code.** The morning reinstall carried a fresh ad-hoc signature and silently voided the grant; the mascot strolled through a streaming response. This is item 38 predicting itself. Two signals made the diagnosis quick and are now written into `docs/QA_CHECKLIST.md`: `Claude.app`'s mtime and version showed the marker could not have moved since the last passing run, and **a strolling mascot means nothing was seen at all**, where a typing one would mean a real agent turn outranked the chat. macOS also keeps listing the app as granted while the signature no longer matches — the entry must be removed and re-added.
- **Every remaining negative and interaction row then passed in one sitting.** A quiet chat produces no pose; a working Claude Code turn outranks a streaming response; an idle session does not block one; the navy mascot is unaffected; the pose holds for a whole stream; the choice survives relaunch. **The idle/working pair is the important one** — the suppression rule removed on 2026-08-11 would have passed "working outranks" while failing "idle does not block", and passing only one of those is exactly how the feature became unreachable for anyone running Claude Code.
- **Two rows stayed unticked on purpose, and both are narrower than they look.** The Thinker pose has now been seen twice and **timed zero times**; that row claims under one second. And only the *persisted* half of the toggle row is done — unticking mid-stream to watch the pose drop at once has not been reported. Seeing a pose is not the same claim as either.
- **Group C was reported as working "according to description" and was left unticked.** Inference is not observation, and this file has been wrong in that exact way before — nine display-matrix rows sat ticked for a day on a blanket verdict nobody had walked, and walking them found a real defect.
- **Exact next step:** the core app smoke test, which is now the largest untouched block, plus the two narrow chat rows above. Behind those: the fixture-only `failed` state and issue #11's Reduce Motion scope. Repoint the local remote — the classifier refused it three times for the assistant.

### 2026-08-13 — The Claude Code hooks never ran, and the Codex fix had already said why

Found from the outside in. The owner reported the mascot idle during a working session and, reasonably, read it as project-scoped: "it only works for this clawd sessions."

- **The report was right about the symptom and wrong about the cause.** Nothing is project-scoped. `~/.claude/settings.json` carried `"args": ["--hook", "--provider", "claude-code"]` on all eight dockpet handlers, and **Claude Code has no `args` field on a command hook** — it runs `command` through the shell and drops every other key. The helper launched with no arguments, hit `missingProvider`, and exited 64. **Every hook, every session, every project, since the hooks were installed.**
- **Reproduced in one command** before anything was edited: piping a payload to the bare helper printed `dockpet-event: missingProvider` and returned 64. That is the whole defect, and it was visible without reading any Swift.
- **The generator wrote it.** `dockpet-event --print-hooks --provider claude-code` emitted the broken snippet verbatim, which is how it reached the settings file — `.bak-dockpet-*` files in `~/.claude/` date the install. **PokeDesk never wrote that file, and still doesn't; it wrote the text the owner pasted, which is the same reach with an extra step.**
- **This defect had already been found and fixed once, for Codex, on 2026-08-02.** The comment explaining it sat six lines below the code still doing it: Codex "does not define Claude Code's separate `args` field", so the fix was filed as a Codex quirk. **The premise was never checked against Claude Code's own reference** — the claim that it "models argv separately from the executable" was written, believed, and shipped. Both providers take one shell command string. The `switch` is now gone; there was never a difference to branch on.
- **The tests encoded the bug and passed because of it.** `theSnippetIsValidJSONNamingTheHelperAndProvider` asserted `--provider", "claude-code` — the args-array spelling — so the suite defended the defect. The Codex regression test was correct and correctly named, and covered one provider. It is now parameterized over both.
- **Verified end to end, not just green:** the hook line exactly as `settings.json` now spells it, fed a real payload, printed `sent active for claude-code` and exited 0. 75 transport tests pass.
- **The reusable lesson, and the 2026-08-12 rename session logged one the same shape ("a fix that had already shipped"):** a fix that lands on one provider is a claim about *both* until the other is checked. Filing it as a quirk of the provider that surfaced it is what kept the bug alive for eleven days — and the test that would have caught it existed, aimed at one argument. **When a bug is provider-shaped, parameterize the regression test over providers before believing the fix.**
- **A hook that fails is silent by construction.** `main.swift` is careful never to fail a user's session, and usage errors are the one exception it makes loud — into stderr nobody reads. **Exit 64 and exit 0 looked identical from the mascot's side**, which is why an unreachable mascot and a misconfigured one need to stay distinguishable somewhere the owner can see.
- **Exact next step:** the core app smoke test, unchanged from yesterday. Behind it: the two narrow chat rows, the fixture-only `failed` state, and issue #11's Reduce Motion scope.

### 2026-08-21 — A cue for the moment the turn stops, and a revoked certificate that ate the app

A small feature session on `fix/hook-flags-in-command`, which still carries the unmerged `args` fix from 2026-08-13. Owner request: a sound on the waiting motion, so a chat window sitting on a permission prompt is noticed, and a sound that is not mistaken for `success`.

- **A fifth cue, `waiting`, and it is the first one attached to a state rather than to a moment.** The existing rule in `MascotSoundPlayer` was explicit that working, waiting, and idle are continuous and a cue for each would make the mascot a noise source. That rule survives for the other two, because what is actually being sounded here is the **edge**: `AmbientAnimationController.adopt` calls `onStateAppeared` only when the displayed state genuinely changes, so a prompt left unanswered for ten minutes knocks once. **The comment saying otherwise was rewritten, not left standing** — a doc comment that argues against the code below it is the next reader's trap.
- **Rhythm, not pitch, is what separates it from `success`.** Two of the four existing cues are rising runs in the same register, so a third rising run distinguished only by interval would fail exactly when the cue matters — heard from another window, not looked at. `waiting` falls instead (B5 → F#5), stops, leaves a beat of silence, and repeats itself at 80%. That puts it two octaves above `failure`, the only other descending cue, and clean where that one deliberately beats.
- **Regeneration stayed deterministic and it was checked rather than assumed.** Re-running `tools/author_sound_effects.py` left `success`, `failure`, `summon`, and `dismiss` byte-identical — `git status` on `art/audio/` shows one untracked file and no modifications. The new WAV measures 4.66s at peak 0.220 FS, the same as the other four; the `write_wav` normalizer was left alone precisely so this stayed true. It was 0.71s until the owner heard it and asked for four to five seconds.
- **Evidence is a build claim and nothing more.** 245 package tests pass, `xcodebuild` succeeds, and `waiting.wav` is in the built bundle's `Contents/Resources/`. **Nobody has heard it fire.** `docs/QA_CHECKLIST.md` gained a "Waiting cue acceptance" section, entirely unticked, and it carries the conditions from 2026-08-10 that make the state actually appear: **Manual** permission mode and a file that **exists**. Auto mode may never prompt and a missing file gets reported rather than asked about — both turn a real failure into "nothing happened".
- **The installed bundle at `~/Applications` does not have this cue** and will not until someone runs `tools/install_app.sh`. Item 32 has now cost this project two wasted observation rounds; the hands-on check below names the install step first for that reason.
- **The install was then blocked as malware, and the cue had nothing to do with it.** The owner ran `install_app.sh`, and macOS moved `~/Applications/Dock Pet.app` to the Trash on launch. Cause, from the unified log at 18:48:38: `CoreServicesUIAgent … moved <private> to trash` immediately after `syspolicyd … GatekeeperPolicyScanError Code=-67018`. The app was signed with **`Apple Development: oaksoekhant05@icloud.com`, and that certificate is REVOKED** — `spctl -a -t exec` says `CSSMERR_TP_CERT_REVOKED`. **A revoked signature is not a weak signature; macOS treats it as malware and deletes the app**, which is why this looked like a virus detection and was not one.
- **Three tools were asked and only one told the truth.** `security find-identity -v -p codesigning` listed all three of the keychain's same-named Apple Development certificates as "3 valid identities found"; `codesign --verify --deep --strict` passed; **`spctl` was the only one that checks revocation**, because it is the only one that goes online to ask. Signing a throwaway binary with each of the three hashes in turn showed **two of them revoked and one not** — and `awk '… {print $2; exit}'` takes the first, which is revoked. **Certificate selection was effectively a coin flip nobody knew was being tossed.**
- **This had been latent since 2026-03-01 and was hidden by an unrelated failure.** All three certificates were issued that day. Every previous install escaped only because `codesign` cannot reach a private key from a non-interactive shell and fell back to ad-hoc — recorded in this ledger three separate times as a harmless quirk (`errSecInternalComponent`). **It was the only thing standing between this project and a deleted app**, and running the script from an interactive terminal removed it.
- **The fallback was removed rather than repaired.** An `Apple Development` certificate cannot satisfy Gatekeeper for a locally launched app under any circumstances — that needs Developer ID plus notarization — so the branch had **zero upside and an unbounded downside**. `install_app.sh` now prefers Developer ID, signs ad-hoc otherwise, re-signs ad-hoc if `spctl` reports a revoked certificate, and **refuses to install a bundle that still fails that check**.
- **Reinstalled and confirmed:** ad-hoc signature, no `CSSMERR_TP_CERT_REVOKED`, `waiting.wav` in `Contents/Resources/`, launched and still running with no new code-evaluation prompt in the log. `spctl` still reports plain `rejected`, which is what every unnotarized app reports and is **not** the block that deleted it.
- **Where the deleted copy went is unconfirmed.** `~/.Trash` listed empty on the first check and returned `Operation not permitted` on the second, so neither answer is evidence. The owner should look in the Trash themselves before assuming it is gone.
- **Owner heard it and approved it: "Nice it work".** That closes the sound itself and nothing else. The rows about *when* it fires — a real Manual-mode permission prompt, no second knock while the prompt sits, a second prompt knocking again, and the two cut-short rows the length introduced — are all still unwalked. **This is the distinction the 2026-08-12 entry was written about**: a blanket "it works" is one observation, not eleven.
- **Both changes landed on `main` under a PR that describes neither of them.** The branch already had an open [PR #53](https://github.com/Mr-Shine09/PokeDesk/pull/53) for the 2026-08-13 hook-flags fix; a fast-forward push of all three commits to `main` made GitHub mark that PR merged, so the cue and the certificate fix are on `main` with no PR text of their own. Merging by direct push was what the owner asked for. **The 2026-08-12 entry logged the same shape and it is now twice**: this ledger, not the PR list, is the only place those two changes are described. CI is green on `main` and the branch is deleted.
- **The doc claims in this entry were checked against `git show HEAD:<file>`, not against the edit scripts that wrote them** — `CLAUDE.md`'s five-cue line, `README.md`'s toggle row, `ARCHITECTURE.md`'s "exactly five moments", `ASSET_PIPELINE.md`'s "the five cues". Re-running `tools/author_sound_effects.py` against the committed tree leaves all five WAVs byte-identical, and the installed bundle's `waiting.wav` matches the committed file at 411044 bytes. This is `CLAUDE.md` rule 3 being followed rather than cited.
- **Exact next step:** the unwalked rows in "Waiting cue acceptance", against the install that is now on disk. Behind that, unchanged for a week: the core app smoke test, the two narrow chat rows, the fixture-only `failed` state, and issue #11's Reduce Motion scope. This branch also still holds the unmerged 2026-08-13 `args` fix.
...
38. **Chat lifecycle detection reads the Claude window through the accessibility API, by owner decision on 2026-08-11, reversing this project's "no accessibility permissions" promise.** It is **off by default** and **was observed working end to end on 2026-08-11** — Thinker pose during a stream, success reaction on completion, back to strolling. **The negative and interaction rows were walked on 2026-08-12 and all pass** — a quiet chat produces no pose, a working Claude Code turn outranks a streaming response, an idle session does not block one, the navy mascot is unaffected, the pose holds for a whole stream, and the toggle survives relaunch. The feature is now proven in both directions rather than on a single positive run. **Two things remain open and are narrower than they look:** the Thinker pose's latency from Enter has never been *timed* (the pose has been seen twice; the row claims under a second), and unticking the menu item **mid-stream** to watch the pose drop at once has not been reported — only the persisted choice has. What must not be undone by accident:
    ...
    - **`waiting` is unimplemented** — no marker captured. **ChatGPT is unwired permanently, and that is the finished answer, not a deferral:** the ChatGPT desktop app is the Codex app, so its turns already arrive as hook events and drive the navy mascot through the real working/success lifecycle (observed 2026-08-11 on both an agentic and a plain conversational turn). An AX marker for it would be a weaker signal that `working` outranks and nobody would ever see. Do not reopen it as a gap.
    ...
    - `ChatAccessibilityProbe` and the `Chat Detection (Experimental)` menu are temporary scaffolding. Remove them once the feature is proven or abandoned — not before, because they are the only way to tell the two apart. The probe no longer prints `AXTitle` or `AXValue` for any role, and bounds `AXDescription`/`AXHelp` at 80 characters: it leaked a conversation summary out of a *button title* once, and the assumption that labels are structure was simply wrong.
    - The menu item reads **Think While Claude Is Answering**. It said "Think When Chat App Is Open" until the label was corrected the same day — accurate for the frontmost-app observer that `ChatActivityWatcher` replaced, and wrong afterwards. The symbol `chatAppsDriveIdeating` and its defaults key still say `chatApps`; **renaming the key would silently reset every stored choice**, so the mismatch is documented at the property instead.

39. **`EventSurface` widened the event envelope on 2026-08-11 — the first widening, and a recorded owner decision.** The ChatGPT desktop app **is** the Codex app (hence `com.openai.codex`), so a chat turn and a terminal agent run fire the same hooks with the same provider and the same two payload fields. The owner's rule is that the chat interface **thinks** while Codex and the terminal **type**. What must not be undone by accident:
    - **The value is two words** — `desktop-chat` or `command-line` — decided inside the hook helper by `EventSurfaceDetector`, which walks *its own* parent processes. No path, command line, process name, or PID leaves the helper. `README.md` discloses it as an always-on behavior, separately from the opt-in chat feature, because it is not opt-in.
    - **Two conditions, not one, decide the pose.** `MascotStateReducer.isConversing` requires a desktop-chat surface **and** no tool traffic this turn. Surface alone was shipped first and was watched failing: the ChatGPT app hosts Codex and its chat behind one binary and one process tree, so agent runs thought along with the chat. Drop the tool check and that returns; drop the surface check and a terminal run thinks during the gap before its first tool call.
    - **The tool flag is turn-scoped**, cleared by a bare `.active` (a new prompt). A sticky flag would leave the pet typing at a chat for the rest of a session that once ran a tool.
    - **`Claude.app` is deliberately absent from `appBundleMarkers`**, and a test pins its absence. It hosts Claude Code, which is agent work; the earlier case names `app`/`terminal` would have turned every Claude Code turn into a Thinker pose, and the rename to `desktopChat`/`commandLine` is what made the question answerable.
    - **Every unknown reduces to `command-line`** — absent field, unrecognized string, older helper, other provider — so nothing predating the field changes behavior. Do not invert that default.
    - **`envelopeExposesOnlyTheAllowlistedFields` is the tripwire** and it fired correctly. Any future field must break it; that is the point, and updating it is the deliberate act.

40. **The project is PokeDesk in prose and Dock Pet in every identifier, deliberately (2026-08-12).** The GitHub repository is now `Mr-Shine09/PokeDesk`. **Do not "finish" the rename in code.** `~/Applications/Dock Pet.app`, the `dockpet-event` helper, the menu's "Quit Dock Pet", and the bundle identifier `com.mrshine09.dockpet` are the exact strings installed provider hooks and the menu-bar visibility store are keyed to — and a menu-bar removal keyed to a bundle identifier cannot be undone from inside the app (item 24). `EventSocketLocation.directoryName` still spells `desktopmascot` for the same reason, one layer deeper. `DesktopMascot.md` keeps every "Dock Pet" and every old repository link on purpose: it is a dated record of what was decided when, and those entries were written against those names.
    - ~~**The local git remote may still point at `desktop-mascot.git`.**~~ **Done — `git remote -v` reads `https://github.com/Mr-Shine09/PokeDesk.git` as of 2026-08-21.** It stayed on the list for nine days because pushes worked through GitHub's redirect and nothing appeared broken; the classifier had refused `git remote set-url` for the assistant on 2026-08-12, so it was left for the owner. Verified by running the command, not by reading this line.
    - **`claude/eloquent-dhawan-d48dec` holds a superseded alternative placement design and must not be merged.** It is the 2026-08-01 `ScreenPlacement` work — stateless, purely geometric, nearest-display — rescued from an uncommitted worktree eleven days late. **PR #35 already fixed the same focus-dependence bug** with a rule that remembers the last display the panel was genuinely on, and `7a898f3` built on that memory. The branch is kept as a record of the investigation and its reproduction, not as pending work.
    - **Before proposing a fix from an old branch, check whether `main` already has one.** `git log --oneline <base>..main -- <path>` answers it in one command. On 2026-08-12 a grep for `NSScreen.main` found a surviving *fallback* line and it was read as the surviving *defect*; the recommendation that followed was wrong twice before the function around the line was read. **Grep finds a string; only the function tells you what it does.**
    - **`tools/list_merged_branches.sh` reports against your last fetch.** Run it as `--fetch`, or it will faithfully list branches that were deleted on the remote days ago — four of seven, on 2026-08-12.
41. **Neither provider supports an `args` field on a command hook. Both take one shell command string, and the flags belong inside it** (2026-08-13). `HookConfiguration.snippet` no longer branches on provider, and `hookModeGoesInTheCommandRatherThanAnUnsupportedArgsField` is parameterized over both — **do not narrow it back to one.** The Claude Code half of this shipped broken from 2026-08-02 to 2026-08-13 because the identical Codex fix was filed as a Codex quirk, and the snippet test asserted the broken spelling, so the suite protected it. **A helper launched without `--hook --provider` exits 64 on every hook and the mascot simply never moves**, in every project, which reads on screen as "the mascot is broken for this repo" and is not. If the mascot is idle during real work, run the configured hook line by hand with a payload on stdin before investigating anything in the app: `sent active for claude-code` means the socket path is fine and the problem is elsewhere.
42. **Anyone who installed hooks before 2026-08-13 has the broken `args` shape in their settings file, and nothing in the app will tell them.** The owner's own `~/.claude/settings.json` was repaired by hand that day (backup: `settings.json.bak-argsfix-20260813-144124`). A fixed generator only helps the next install; there is no migration, and **`--print-hooks` from an old installed bundle keeps emitting the broken snippet until the bundle is rebuilt.** That is the argument for a menu-bar diagnostic that says "no provider events since launch" — not yet built, and the cheapest thing that would have made this defect self-reporting.

43. **`waiting` has a cue as of 2026-08-21, and it is the only cue attached to a lasting state.** It works because `AmbientAnimationController.adopt` fires `onStateAppeared` only on a genuine change of displayed state — one knock per entry into `waiting`, silence for however long the prompt sits. **Anything that makes that callback re-fire on an unchanged state turns this cue into a repeating alarm**, so treat it as load-bearing rather than as an implementation detail of the animation controller. The sound is a descending pair, deliberately not a rising run: `success` and `summon` both are, and the owner's requirement was that this one not be confused with success when heard from another window. **It runs 4.66 seconds** — four knocks tapering to a stop — because the owner heard the 0.71s first cut and asked for four to five. **The length is the point and it was asked for twice over; do not shorten it back.** That length is also why the cue is **cut short when any other state reaches the screen or the mascot is dismissed**: it can otherwise outlive the prompt it is announcing. It shares the single Sounds toggle, the `isVisible` gate, and the per-state coalescing window with the other reaction cues; a separate toggle was rejected. **It has never been heard against a real permission prompt** — `docs/QA_CHECKLIST.md` → "Waiting cue acceptance" is the open check, and it needs **Manual** mode and a file that exists, or the state never appears.

44. **A revoked signing certificate makes macOS delete the app as malware, and this keychain holds two of them (2026-08-21).** `install_app.sh` used to fall back to any `Apple Development` identity, picked the first of three same-named certificates, and that one was revoked; the installed app was moved to the Trash the moment it launched, with the alert that says the app will damage your computer. **Nothing was infected and nothing in the app was at fault.** What must not be undone:
    - **Do not restore the `Apple Development` fallback.** Such a certificate cannot satisfy Gatekeeper for a locally launched app under any circumstances — that path is Developer ID plus notarization — so it was pure downside. Ad-hoc has no certificate and therefore nothing to revoke.
    - **`security find-identity -v` and `codesign --verify` both call a revoked certificate fine**, because neither performs the online revocation check. **`spctl -a -t exec` is the only one that answers**, and `CSSMERR_TP_CERT_REVOKED` is the string to look for. The install script now runs it and refuses to install a bundle that fails.
    - **`spctl` reporting plain `rejected` is normal** for every unnotarized build here and is not this problem. Only the revoked verdict is.
    - **The evidence is in the unified log, not on screen:** `/usr/bin/log show --predicate 'process == "CoreServicesUIAgent"'` prints `moved <private> to trash` next to `present code-evaluation prompt`, and `syspolicyd` prints `GatekeeperPolicyScanError Code=-67018` alongside. Use `/usr/bin/log` — zsh has its own `log` builtin that swallows the arguments and reports "too many arguments".
    - **The `errSecInternalComponent` fallback was load-bearing and this ledger called it harmless three times.** It is why every earlier install was ad-hoc and survived. A workaround that has been quietly protecting you is not the same as a workaround that does not matter.
