...
- **Exact next step:** watch a real Codex session drive the navy mascot on screen. It is the last 0.1 hands-on claim, the rig is now known to work, and the setup is the same as today's ideating check — summon both mascots, run one real Codex turn, and confirm the navy one reacts while the orange one strolls. Check the installed bundle's date first.
