...
| Window geometry | Automated fixtures plus manual multi-display matrix | Rescoped 2026-07-30: `DockGeometry` is bottom-anchored only and has no Dock-edge inference, but later the same day `WindowCoordinator.settleAfterDrop()` gained a sanctioned override so a dropped mascot roams at the height it landed at, with `reposition()` the one way back to the default lane. 24 tests pass (`swift test --filter MascotWindowTests`), covering placement, clamp-with-guard, visibility, manual-position preservation across Hide/Show, non-activating interaction, click routing, drag begin/end routing, a drop that keeps both axes, an off-edge drop that clamps, a drop past each attached display's edge, a re-settle that does not drift, roaming that preserves a dropped height, and reposition restoring the lane. Extended 2026-08-01 with nine `ScreenPlacement` fixtures that pin which display is chosen, on synthetic arrangements rather than the hardware attached to the machine — a side-by-side pair, a stacked pair with a dead band between them, displays touching along an edge, exact ties, and no displays at all. The hands-on multi-display matrix is still pending: nobody has dragged the mascot between two physical displays |
| Atlas runtime mapping | Every declared row crops to the matching frozen frame pixels | Passed 2026-07-29 for offline, idle, working, ideating, both walk directions, both chair sit rows, and hanging |
...
| Placement follows keyboard focus instead of the mascot | High | Found 2026-08-01: `horizontalMovementBounds()`, `verticalPlacementBounds()`, and `reposition()` all resolved their display as `panel.screen ?? NSScreen.main`. `NSScreen.main` is the display holding the key window, so with a second display attached the display a drop was clamped against depended on where the user had last clicked, and a mascot dropped past an outer edge — where `panel.screen` is `nil` — could be clamped onto a display it was never dragged to. Fixed by `ScreenPlacement`, which answers from the panel frame and the display arrangement alone; `NSScreen.main` no longer appears anywhere in `MascotWindow` | Resolved 2026-08-01. The old failure was reproduced exactly against live screen geometry before the change; nine arrangement-independent fixtures pin the new rule. Hands-on multi-display QA is still open, so the *feel* of dropping onto a second display is unverified |
...
- Reposition relative to the panel's current display, falling back to the main display only when the panel has no screen. *(Superseded 2026-08-01: the fallback to the main display was the defect. See the next entry.)*
- Choose the display for all placement, clamping, and backing-scale rounding from the panel's own frame and the display arrangement — never from `NSScreen.main`. The rule is: the display the panel covers most of; when it covers none, the nearest display measured from the panel's centre; ties to the lowest index. `NSScreen.main` names the display holding the key window, so the previous `panel.screen ?? NSScreen.main` made the same drop on the same arrangement land in two different places depending on where the user had last clicked, and could clamp the mascot onto a display it had never been dragged to — which the 2026-07-28 rule "never teleport across unrelated displays unless the user selects a display" already forbade. Nearest-display was chosen over "the display it was on before the drag" so that no placement state has to be carried across a drag, a wake, or a display change: the answer is a pure function of geometry at the moment it is asked.
...
### 2026-08-01 — Placement stops following keyboard focus

- Objective: decide how `WindowCoordinator.settleAfterDrop()` should choose the display it clamps against, after `aDropPastAnEdgeIsClampedBackIntoView` was reported failing on `origin/main` whenever a second display is attached — order- and focus-sensitive, passing alone under `--filter` and on a single display. Pre-existing, unrelated to the dismiss-transition work on `agent/dismiss-poof-and-app-icon`.
- Reproduced first, exactly. The machine has two displays: built-in Retina `(0, 0, 1280, 832)` with visible frame `(0, 66, 1280, 736)`, and a Sidecar display `(1280, -2, 1194, 834)` whose visible frame reports `maxY 801` once AppKit is initialised. The test samples bounds while the panel is on the built-in (`x ≤ 1184`, `y ≤ 690`), moves the panel 500 points past both, and settles. At `(1684, 1190)` the panel overlaps **no** display, so `NSWindow.screen` is `nil` and `NSScreen.main` decides: built-in gives `1184, 690` and passes; Sidecar gives `1684, 689` and fails. Those are the two numbers in the report, to the point. Two further facts came out of the same probe and are worth keeping: `NSWindow.screen` is **not** `nil` for a window that is merely ordered out — it resolves from the frame — and a display's `visibleFrame` can differ between a plain tool and an AppKit app process, which is where the odd one-point `689 vs 690` came from.
- Decision: choose the display from the panel's frame and the display arrangement, never from focus. Recorded in the Decision log; it supersedes the 2026-07-28 "falling back to the main display" line, whose fallback was the defect.
- Completed:
  - Added `MascotWindow/ScreenPlacement.swift`: pure `CGRect` arithmetic returning the index of the display a panel belongs to — largest overlap, else nearest to the panel's centre, ties to the lowest index.
  - `horizontalMovementBounds()`, `verticalPlacementBounds()`, `settleAfterDrop()`, and `reposition()` all resolve through it. `settleAfterDrop()` now resolves **one** display and clamps both axes against it, rather than resolving per axis, which could in principle have produced a position on neither display.
  - Added `placementBackingScaleFactor` and used it for the walk's pixel rounding in `AmbientAnimationController`, so rounding and clamping cannot disagree about which display is in play.
  - `NSScreen.main` now appears nowhere in the sources except in comments explaining why it is not used.
- Test changes, and why the old assertion had to go: comparing a clamp result against bounds sampled *before* the panel moved is only valid on a single display. `1184, 690` was never the right expectation for a panel stranded above the *second* display — the geometry answer there is the Sidecar, and the built-in answer the test wanted was the accidental one. `aDropPastAnEdgeIsClampedBackIntoView` now pushes the panel past every corner of the whole desktop and asserts the invariant that actually matters — the panel ends inside the bounds of whichever display it settled against, and intersects a real display — read *after* the settle. Two tests joined it: one that repeats this past each attached display's own edges (so a second display is exercised when one is present), and one that settles three times and expects no drift, since `screenParametersChanged` re-settles on every display change and wake.
- Verification: 187 package tests pass (was 176; +9 `ScreenPlacement` fixtures, +2 `WindowCoordinator`), run three times with no order sensitivity; `swift test --filter MascotWindowTests` is 24. Atlas contract and atlas validate, `git diff --check` is clean, unsigned Debug and Release Xcode builds succeed. The nine new fixtures run on synthetic arrangements — a side-by-side pair using this machine's real numbers, a stacked pair with a 200-point dead band, displays touching along an edge, exact ties on both the overlap and the nearest path, a single display, and none — so they do not depend on what is plugged in.
- What is **not** proven: nobody has dragged the mascot between two physical displays. The claim here is that placement is now deterministic and cannot follow focus, which is a structural property — the code no longer reads the focus-dependent input — plus fixtures pinning the geometry. It is not a claim that dropping onto a second display *feels* right, or that unplugging a display behaves well on screen. Three specific hands-on checks were added to `docs/QA_CHECKLIST.md` under the display matrix.
- Risks or blockers: none introduced. One behavior change worth watching during that QA: a mascot dismissed on one display and summoned back now returns to the display it was on, where before it could reappear on whichever display had since taken focus. That is the intended reading of "never teleport across unrelated displays", but it is a change, and it is only visible with two displays connected.
- Next: unchanged — owner hands-on QA, cues first, then drop-from-heights, then the display matrix, which now has three more rows worth exercising. Plus the outstanding animation-speed-control decision.
