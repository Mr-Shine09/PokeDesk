...
37. **"Make the mascot stay in one place" is an existing feature, not a gap.** Unchecking **Roam Along Bottom** holds the pet where it is, keeps it animating in place, persists across relaunch, and leaves reactions alone because `setRoaming` acts only when `plan.isAmbient`. It was requested as new work on 2026-08-11 because the menu names what is switched *off* rather than the mode switched *on*. The README now spells it out. **Check whether a requested feature already exists before building it — by reading the code that would implement it, not the menu title.**
