...
4. `main` is at `4a1dbd7` and includes everything through [PR #25](https://github.com/Mr-Shine09/desktop-mascot/pull/25). No PR is open and no branch is outstanding. Verify with `git status --short --branch` and `gh pr list` rather than trusting this line — it is a snapshot, and it has been wrong before.
