...
| Current gate | Drive animation selection from `MascotVisibleState`, replacing the raw atlas row strings in `AmbientAnimationController`, with ambient roaming as the no-signal default. The menu-bar event readout is owner-verified; hands-on QA still outstanding for the physical cursor-hanging feel and the display matrix |
