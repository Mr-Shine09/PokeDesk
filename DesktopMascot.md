...
| Status | Portal summon, revision 4 chair sit-shake/cursor-hanging atlas, bottom-anchored window placement, ambient animation, event engine, and local transport are merged to `main` (`a351aed`). The app now runs the event socket server and surfaces reduced state in menu-bar diagnostics on branch `agent/event-server-app-wiring` (`bc6af4f`), not yet merged |
| Current gate | Drive animation selection from `MascotVisibleState`, replacing the raw atlas row strings in `AmbientAnimationController`, with ambient roaming as the no-signal default. Owner hands-on QA still outstanding for the menu-bar event readout, the physical cursor-hanging feel, and the display matrix |
...
| [#7 Build the private local event bridge and helper CLI](https://github.com/Mr-Shine09/desktop-mascot/issues/7) | 3 | In progress; `MascotTransport` and the `dockpet-event` helper are implemented, unit-tested, and verified cross-process on 2026-07-29. The app runs the server and surfaces reduced state in menu-bar diagnostics as of 2026-07-30. Remaining: bundle the helper so a provider hook can invoke it, and owner confirmation of the menu-bar readout; GitHub status sync pending |
...
| Hide/Show discards a manually dragged position | Medium | Found 2026-07-29 during maintainer onboarding: `WindowCoordinator.setVisible(true)` repositioned unconditionally. Fixed 2026-07-30 with a `repositioning` parameter that `AppDelegate` sets from `isRoaming` | Resolved 2026-07-30; 118-test suite includes fixtures for both the preserved and the reset case |
...
- Risks or blockers: the menu-bar readout itself was **not** verified this session. Reading a `MenuBarExtra` needs assistive access, which was not granted, and granting it is a security-settings change this session did not make. So the strings are unproven on screen even though the values behind them are tested — that is owner QA, not a passed check. The helper is still not bundled anywhere a provider hook could invoke it, so the path remains developer-only end to end.
...
4. `main` and `origin/main` are level at `a351aed`. Branch `agent/event-server-app-wiring` (`bc6af4f`) holds the app-side event server wiring and is unmerged and unpushed as of 2026-07-30. Verify with `git status --short --branch` and `gh pr list` rather than trusting this line.
...
7. Preserve the honest capability boundary, which moved on 2026-07-30 but did not disappear. The app now listens on the socket and reduces real events into `MascotVisibleState`, and shows that state in the menu bar. But **nothing consumes it**: animation is still ambient roaming that knows nothing about ChatGPT or Claude, and **no provider hook can reach the helper**, because the helper is not bundled and no adapter exists. So the only way an event reaches Dock Pet today is a developer running `dockpet-event` by hand. Do not describe Dock Pet as reflecting real agent activity. The next steps are animation selection, then bundling the helper, then adapters #8 and #9.
