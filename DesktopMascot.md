...
| Current gate | Owner hands-on QA. The new Codex fashion atlas (dark sunglasses, orange/white top, unchanged gray trousers and navy shoes) is implemented and structurally verified but awaits owner visual approval. All four sound cues, the dismiss/quit transition, drag-and-drop, and the app icon were approved on 2026-08-01 from the installed `~/Applications` build. What remains: the Codex wardrobe visual check, then the **display matrix**, which has a known multi-display clamping defect. Animation speed, launch-at-login, and distribution are closed by owner decision |
...
- ~~Whether Claude Code and Codex deserve distinct visual accents.~~ Resolved 2026-08-01: Codex uses the orange/sunglasses wardrobe; Claude Code retains the original wardrobe.
...
| Provider-specific fashion | Codex attribution selects the orange/sunglasses atlas; Claude/no-provider selects classic; both atlases preserve cell geometry and alpha | Passed 2026-08-01: deterministic generation is byte-reproducible, all 16 rows passed visual contact-sheet review, per-frame alpha equality passed, `poof` is pixel-identical, three provider-selection fixtures pass, and the isolated Debug bundle carries a byte-identical Codex atlas. Owner visual approval and a live Codex hook run remain pending |
...
- Next: owner reviews `art/animation/qa/contact-sheet-codex-fashion-4x.png`, then the existing reaction-cue and display-matrix QA queue resumes.
...
3. Treat atlas revision 6 as the geometry/timing contract: 16 rows, `768x1792`. `mascot-atlas@2x.png` is the classic Claude/no-provider wardrobe and `mascot-atlas-codex@2x.png` is its deterministic orange/sunglasses Codex derivative. Row 15 is the eight-frame `poof` smoke cloud and row 14 the four-frame `hand-sign` dismiss seal. `poof` is the only row that is not the character and is pixel-identical across both atlases; the hanging row remains at index 13 with a `(48, 4)` grip anchor.
...
20. Provider wardrobe selection consumes `MascotVisibleState.providers`; it does not bypass the reducer. Classic is Claude/no-provider/manual, Codex is orange/sunglasses, and Codex wins only the visual accent when both providers contribute. A live Codex run and owner visual approval are still pending.
