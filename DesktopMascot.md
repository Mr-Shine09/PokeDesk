...
31. **Three behavior changes shipped on 2026-08-09 with no on-screen observation at all** — the waiting timeout, the ideating reorder, and the sleep schedule. Every one is proven by unit test and a green build only. Do not describe them as verified. The cheapest checks are: set the sleep hour to the current hour and watch the pet lie down; toggle Manual Ideating while an agent is running and look for the lightbulb; leave a real permission prompt unanswered for more than two minutes and confirm the pet stays put. **They were not merely undone until 2026-08-10 — they were impossible**, because the installed bundle predated all three changes. It was reinstalled from `main` that day, so the checks now exercise the real code.
