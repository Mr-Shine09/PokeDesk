...
22. The animation loop was throttled on 2026-08-05: the ambient tick is **12 Hz**, transitions stay at 20 Hz, the timer carries tolerance, and the loop stops while the panel is occluded. Transitions and drags are exempt from the occlusion pause on purpose — `beginDismiss` orders the panel out from its completion and Quit waits on that, so pausing a dismiss would make the app unquittable. Do not extend the pause to cover them. Measured 2026-08-05 at **2.17% median with one mascot**, down from 3.40%, against a <1% gate — improved, still failing. Animation cost is close to linear in tick rate, so closing the gate this way would need ~5.5 Hz, below the sprite's own frame rate. Do not cut the rate further without an owner decision; restating the gate is the recommendation. The 12 Hz walk has not been looked at by anyone.
