...
- Verification: after the fix the app persists `NSStatusItem VisibleCC DockPetMenuBarItem = 1` and stays alive past 20 s. The stale `Item-0 = 0` remains in the domain as the dead identity and is harmless. Package tests unaffected. **Owner on-screen confirmation of the icon was still pending when this was committed.**
