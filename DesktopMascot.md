...
| Current gate | **Release gates only — 0.1 feature work and hands-on QA are both closed.** The idle-CPU gate **failed** its first measurement on 2026-08-02 at a 3.40% median against a <1% target; the animation loop was throttled on 2026-08-05 (20 → 12 Hz ambient, timer tolerance, occlusion pause), measured at **2.17% median — improved but still failing by more than 2x**, and not reachable by tick rate alone without visibly degrading the animation. The 12 Hz walk and the occlusion pause on other screens were both owner-verified on 2026-08-05 and are accepted; the gate itself is now an open owner decision between a structural change and restating it. Also open: event-to-visible-state latency, Reduce Motion sign-off, signing, notarization, and packaging. One hands-on claim remains: a real Codex session driving the navy mascot on screen |
| Repository | [Mr-Shine09/desktop-mascot](https://github.com/Mr-Shine09/desktop-mascot) — **still private.** Public-release documentation, licensing, and CI landed 2026-08-08; visibility has not been changed. Blocking: a first real CI run. The username scrub across 12 tracked files was completed 2026-08-09 |
...
- Scope found by `git grep -l "/Users/oaksoekhant"`: **12 tracked files**, not the 11 the previous entry recorded — the ten `art/animation/frames/*/normalization.json` records, plus `art/animation/frames/idle/authoring.json`, which the earlier count missed because it is not named `normalization.json`, plus this ledger.
...
- Data rewrite: the `/Users/oaksoekhant/Mac Dock Pet/` prefix was stripped from all eleven art records. Every path in them was inside the repository, confirmed before editing — no record referenced anything outside it.
...
- Verified, not assumed:
  - `git grep oaksoekhant` returns nothing across the whole tree, including this ledger.
...
7. Preserve the honest capability boundary. Claude Code is observed live end to end. Codex's command-shape defect was fixed on 2026-08-02; its seven hooks are installed, trusted, Active, and exercised by a fresh real turn with no hook failure, and the helper/socket path passed independently. What remains unproven is the navy mascot's on-screen response and real-provider `waiting`/`failed`; do not promote the successful hook run into visual QA that nobody watched.
...
23. **The idle-CPU gate is the one live release decision.** Measured 2.17% median with one mascot (n=20 over 5 min) after the throttle, down from 3.40%, against a <1% gate — cost is near-linear in tick rate, so closing the gap needs ~5.5 Hz, below the sprite's own ~8 fps. The remaining levers are structural (stop moving the `NSPanel` every tick while walking; coalesce both mascots onto one timer, which only helps the two-mascot case) or restating the gate. Restating is the standing recommendation on the evidence. Two mascots have still never been measured; do not assume the cost is linear in mascot count. Sample for minutes — a single 5-second `powermetrics` window landed on the top of the range and produced a wrong conclusion on 2026-08-05.
24. Public-release documentation landed 2026-08-08: root `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, MIT `LICENSE`, a separate restrictive `art/LICENSE.md`, `.github/` templates, and `.github/workflows/ci.yml`. **The username scrub is done (2026-08-09)** — the eleven art provenance records now carry repository-relative paths, this ledger's references are placeholders, and the two tools that write those records emit relative paths so a regeneration cannot reintroduce the leak. Do not "fix" a provenance path back to an absolute one. **What still blocks flipping visibility is CI, which is not green yet.** It ran for the first time on 2026-08-08: the art and build jobs pass, and the test job exposed a real flake — `makePipe` in `HookPayloadReaderTests.swift` dispatched its blocking writer to `DispatchQueue.global()`, which starves on a low-core runner and times out a *different* pipe test each run. Fixed by moving the writer to a dedicated `Thread`; the fix is unverifiable on this machine, since the suite passed both before and after it. Watch the test job over several runs before calling it settled, and note that a `pull_request` workflow runs against `refs/pull/N/merge` and is skipped **silently** while the PR conflicts. The README also has no screenshot, which wants an owner capture rather than a generated one. Keep the README's "Current limitations" section honest: it names the failing 2.17% idle-CPU gate, the missing notarized build, the unobserved Codex on-screen reaction, and the fixture-only `waiting`/`failed` states, and those must not be softened to make the project page look better.
