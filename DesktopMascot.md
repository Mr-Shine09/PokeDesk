...
34. **The README screenshot has a written procedure and is waiting only on the capture.** `docs/ASSET_PIPELINE.md` → "Project page screenshot" carries the setup order, the region-capture command, the target path `docs/images/readme-hero@2x.png`, and the accept/reject rule. Three of its five setup steps exist because of mistakes this project already made: reinstalling mid-capture would quit the app under test (item 32), Manual Ideating left on would override the state being shown (item 29), and a blank capture from a process without Screen Recording permission looks like a real PNG (item 33). **Do not add the `![…]` link to `README.md` before the image file exists** — the repository is public and a broken image is worse than no picture.
35. **Unplugging a display loses a mascot's dragged height, and three places in the project say it does not** — `docs/QA_CHECKLIST.md`, this file's risk register, and `WindowCoordinator.screenParametersChanged`'s own doc comment, which now carries a contradicting note. Observed 2026-08-10 with the pet dragged near the top of the external display. Clamping does not explain it: the owner's two displays have aligned top edges, so that height fit the built-in untouched. The hypothesis — AppKit relocating the window before our handler runs, so `settleAfterDrop()` adopts the system's position — is **inferred from source, not established**. Settle it with the cheap check in the matrix row (change resolution or arrangement with a pet dragged high on the *built-in*, which posts the same notification without removing a display) before writing any fix. And do not assume the documented behavior is the wanted one: reappearing at the bottom may be better, and that is an owner question.
