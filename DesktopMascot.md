...
| Current gate | Owner hands-on QA: click through the new menu (especially Preview State), the cursor-hanging feel, and the display matrix (auto-hide, multiple displays, full-screen Spaces, sleep/wake). That matrix is the last thing standing between this and a finished 0.1. Launch-at-login and distribution are closed by owner decision |
...
- Next: owner listens to both cues and drops the mascot from several heights. Ledger rows at lines 1402 and 1407 still claim no adapter exists and no installed copy exists; both were overtaken by PRs #21/#22 and need correcting, as does `docs/HANDOFF.md` line 119.
